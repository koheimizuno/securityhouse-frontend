...
const TabVertical = ({
  ...
  return (
    <div
      className={`w-full flex flex-col items-center mt-6 lg:flex-row lg:items-start ${
        gap && "gap-6"
      }`}
    >
        ...
        {roomCat.map((item, index) => (
          <li
            ...
            className={`border sm:flex sm:flex-row sm:items-center sm:gap-2  p-3 lg:px-6 lg:h-[72px] lg:gap-4 ${
              gap
                ? "rounded-xl"
                : "rounded-t-lg lg:rounded-none lg:rounded-l-xl"
          ...
          >
            <div className="lg:w-[50px]">
              <Image
                src={
                  tab === (index + 1).toString() ? item.icon[1] : item.icon[0]
                }
                alt={
                  tab === (index + 1).toString()
