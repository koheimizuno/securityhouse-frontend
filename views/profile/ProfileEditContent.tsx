    ...
    <>
      ...
      <form className='mt-4 flex flex-col gap-8' onSubmit={handleSubmit}>
        ...
        <label className='flex flex-col gap-2'>
          ...
          <InputText name='display_name' placeholder='山田太郎' onChange={handleChange} />
          {errors.display_name && <span className='text-danger text-sm'>{errors.display_name}</span>}
        </label>
        <label className='flex flex-col gap-2'>
          <span className='text-sm font-bold'>自己紹介</span>
          <TextAreaText name='bio' placeholder='自己紹介を入力' onChange={handleChange} />
          {errors.bio && <span className='text-danger text-sm'>{errors.bio}</span>}
        </label>
