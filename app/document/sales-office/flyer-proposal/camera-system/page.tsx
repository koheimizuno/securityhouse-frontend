...
const CameraSystemPage = () => {
  const { session_user_id } = useAuthentication()
  const [cameraSystemDocData, setCameraSystemDocData] = useState<DocumentType[] | null>([])

  useEffect(() => {
    getDocumentsAction({ type_id: 1, category_id: 5, user_id: session_user_id }).then(data =>
      setCameraSystemDocData(data)
    )
  }, [session_user_id])
    ...
    <Container className='py-16 flex flex-col gap-12'>
      ...
      <section className='flex flex-col gap-6'>
        <div className='flex flex-col md:flex-row md:flex-wrap items-center gap-4'>
          {cameraSystemDocData &&
            cameraSystemDocData.map((doc, key) => (
...
export default CameraSystemPage
